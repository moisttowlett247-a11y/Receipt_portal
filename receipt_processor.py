...
class SubscriptionLicenseManager:
    ...
    Enforces hardware machine locking, sync with local license_registry.json,
    and prevents re-activation of already-active or revoked licenses.
    """
    ...
    def load_registry(self) -> list:
        ...
        # 1. Check local registry file in application directory
        ...
        # 2. Check remote registry URL if configured in .env (e.g. LICENSE_REGISTRY_URL)
    ...
    def save_registry(self, registry_list: list):
        """Saves updated registry (e.g. marking key in_use and locking hardware_id)."""
        try:
    ...
    def sync_with_registry(self):
        """Checks if current local license has been revoked or toggled to inactive in registry."""
        current_key = (self.license_data.get("license_key") or "").strip().upper()
        if not current_key or self.license_data.get("status") != "ACTIVE":
            return
        ...
        if matched:
            status = matched.get("status", "ACTIVE").upper()
            if status in ["NOT ACTIVE", "REVOKED", "SUSPENDED"]:
                ...
                # Check machine lock
                reg_hw = matched.get("hardwareId", "")
                if reg_hw and reg_hw != self.hardware_id:
                    self.license_data["status"] = "REVOKED"
                    self.save_local_license()
    ...
    def verify_with_server(self, license_key: str, email: str = "") -> tuple:
        ...
        # 1. Check live remote server if configured
        if "youraccountingdomain.com" not in LICENSE_SERVER_URL:
            payload = {
            ...
            try:
                resp = requests.post(LICENSE_SERVER_URL, json=payload, timeout=8)
                if resp.status_code == 200:
                    data = resp.json()
                    if data.get("valid"):
                        ...
                        self.license_data["plan_tier"] = data.get("plan", "Custom")
                        self.license_data["activated_at"] = data.get("activated_at", datetime.now().strftime("%Y-%m-%d %H:%M"))
                        self.license_data["expires_at"] = data.get("expires_at", (datetime.now() + timedelta(days=30)).strftime("%Y-%m-%d"))
                        self.license_data["last_verified"] = datetime.now().isoformat()
                        self.save_local_license()
                        return True, f"Subscription Verified Active! Plan: {self.license_data['plan_tier']} • Expires: {self.license_data['expires_at']}"
                    else:
                        self.license_data["status"] = "REVOKED"
                        self.save_local_license()
                        return False, data.get("reason", "License key is invalid or has been revoked.")
            except Exception:
                pass

        # 2. Check local license_registry.json
        registry = self.load_registry()
        if not registry:
            return False, (
                "Verification Failed: 'license_registry.json' not found in application folder.\n"
                "Please export 'license_registry.json' from your Admin Dashboard and place it in this directory."
            )

        matched_idx = -1
        matched = None
        for idx, rec in enumerate(registry):
            if str(rec.get("key", "")).strip().upper() == clean_key:
                matched_idx = idx
                matched = rec
                break

        if not matched:
            return False, f"License Key Not Found: '{clean_key}' does not exist in authorized registry."

        # Check if Admin or Operator marked it NOT ACTIVE / REVOKED
        status = str(matched.get("status", "ACTIVE")).upper()
        if status in ["NOT ACTIVE", "REVOKED", "SUSPENDED"]:
            self.license_data["status"] = "REVOKED"
            self.save_local_license()
            return False, "Access Denied: This license key has been marked 'NOT ACTIVE' (Revoked) by administrator."

        if status == "EXPIRED":
            self.license_data["status"] = "EXPIRED"
            self.save_local_license()
            return False, f"Access Denied: This license key expired on {matched.get('expiresDate', 'N/A')}."

        # Check if already in use
        is_already_in_use = matched.get("inUse", False)
        registered_hw = matched.get("hardwareId", "")

        if is_already_in_use:
            if registered_hw == self.hardware_id:
                return False, f"Notice: This license key is already activated on this computer (Expires: {matched.get('expiresDate', 'Lifetime')}). Re-activation of an already activated key is prohibited."
            else:
                return False, f"Activation Blocked: Key is already in use on another computer ({registered_hw or 'Workstation'}). Multi-machine sharing is restricted."

        # Determine Plan & Duration
        plan = str(matched.get("plan", "")).upper()
        is_admin_perpetual = plan == "ADMIN" or "Never" in str(matched.get("expiresDate", ""))

        now_dt = datetime.now()
        activated_str = now_dt.strftime("%Y-%m-%d %H:%M")

        if is_admin_perpetual:
            expires_str = "Never (Lifetime / Non-Expiring)"
            plan_name = "Admin Master (Never Expires)"
        else:
            duration_days = 30
            if plan == "DEMO": duration_days = 7
            elif plan == "3MONTH": duration_days = 90
            elif plan == "6MONTH": duration_days = 180
            elif plan == "ANNUAL": duration_days = 365
            expires_str = (now_dt + timedelta(days=duration_days)).strftime("%Y-%m-%d")
            plan_name = f"{matched.get('plan', 'Standard')} ({duration_days} Days)"

        # Update and lock in registry
        matched["inUse"] = True
        matched["hardwareId"] = self.hardware_id
        matched["activatedDate"] = now_dt.strftime("%Y-%m-%d")
        matched["expiresDate"] = expires_str
        registry[matched_idx] = matched
        ...
        return True, f"✅ License Successfully Activated! Plan: {plan_name} • Machine Locked • Expires: {expires_str}"
